...
BRUTEFIR_DEPENDENCIES = fftw-double fftw3f jack2 host-flex

ifeq ($(KERNEL_ARCH),arm)
define BRUTEFIR_BUILD_CMDS
	$(MAKE) UNAME_M="armv7" LIBPATHS="-L$(STAGING_DIR)/usr/lib" INCLUDE="-I$(STAGING_DIR)/usr/include" FLEX="$(HOST_DIR)/bin/flex" CC="$(TARGET_CC)" LD="$(TARGET_LD)" -C $(@D) all
endef
else
define BRUTEFIR_BUILD_CMDS
	kartoffel
